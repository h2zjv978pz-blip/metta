...
// Start simple player
    ...
    $('body').on('click', function (e) {
        if ($('.menu-ul').hasClass('active') && !e.target.closest('.hidden-menu-bar') && !e.target.closest('.menu-ul span') && !e.target.closest('.menu-ul')) {
            $('.menu-ul').removeClass('active');
        }

        if (e.target.closest('.lib-dropdown')) {
            if ($(e.target).is('.lib-dropdown > a') && $('.dropdown-content').css('display') === 'flex') {
    ...
    //--------Slide Menu start
    $('.hidden-menu-bar').on('click', function () {
        $('.menu-ul').toggleClass('active');
        console.log('NOICE 02');
    });
    ...
    if (document.getElementsByClassName('mixitup-container').length) {
...
document.addEventListener('DOMContentLoaded', function() {
    // Generic share button handler
    document.querySelectorAll('.share-btn').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();

            if (this.getAttribute('data-share-url')) {
                var shareUrl = this.getAttribute('data-share-url');
            }
            else {
                var shareId = this.getAttribute('data-share-id');
                var shareParam = this.getAttribute('data-share-param') || 'shareId';
                var shareUrl = window.location.protocol + '//' + window.location.host + window.location.pathname + '?' + shareParam + '=' + shareId;
            }
            ...
            navigator.clipboard.writeText(shareUrl).then(function() {
                ...
                if (!toastEl) {
                    // Create toast container and element dynamically if not present
                    var container = document.createElement('div');
                    container.className = 'toast-container position-fixed top-0 end-0 p-3';
                    container.style.zIndex = 1055;

                    toastEl = document.createElement('div');
                    toastEl.id = 'shareToast';
                    toastEl.className = 'share-toast toast animated-toast bg-dark align-items-center border-0';
                    ...
                    toastEl.innerHTML = `
            <div class="d-flex">
              <div class="toast-body">Shareable link copied to clipboard!</div>
          ...
          `;
                ...
                var shareToast = new bootstrap.Toast(toastEl);
                shareToast.show();
            }).catch(function(err) {
